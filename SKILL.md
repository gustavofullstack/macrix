---
name: macrix
description: macOS automation MCP server (107 tools) with concurrent multi-agent access. Use when the user needs calendar, reminders, notes, shortcuts, files, git, web screenshots, system probes, or Jev/TypeSafe judgments without the 100-calls/day commercial cap.
---
...
107 tools in families: Apple apps (calendar, reminders, notes,
...
Harness (v0.26): `agents_list`, `agent_run` (headless prompt on claude_fable/opus/sonnet, codex, antigravity, opencode, muse, goose inside ~/Projetos, ~/Documents or /tmp), `agent_route` (Jev picks the lane: hardest→fable, medium→opus, simple→sonnet, bulk→muse; execute=true runs it), `agents_gate` (in-flight runs, quota-suspended lanes), `env_inventory` (census of MCPs/skills/plugins/hooks/commands). agent_run: max 2 in flight, op_id dedupe 10 min, 429 suspends the lane 30 min, timeout kill = unknown, ledger ~/.config/macrix/agent-ledger.jsonl.
