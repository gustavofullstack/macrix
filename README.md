...
Automação do macOS + computer-use + metering de uso — servidor MCP aberto (open-core). **v0.33.0: 109 tools**, console web e medição por chave. Sem o teto de 100 chamadas/dia do Macuse, com quantos agentes quiser ao mesmo tempo.
...
**Ledger canônico (v0.31):** o MACRIX não reimplementa contabilidade. Com `~/.config/macrix/ledger.json` (`python` ≥ 3.11, `bridge` = `scripts/ledger_bridge.py` do triqhub-os fixado por SHA num worktree, `database`, `tenant`, `attempt_units`), cada `agent_run` faz `reserve → dispatch` antes de o processo existir e `settle` (unidades de tentativa, `price_version=attempt-units-v0`) ou `mark_unknown` (kill por timeout: reserva preservada, conta congelada até revisão) depois. Conta congelada recusa novas tentativas. O tenant é separado do do JEV. Sem o arquivo, o ledger fica desligado e o `agents_gate` diz isso.

**Endurecimento pós-revisão (v0.32):** um só executor (`Harness.execute`) para `agent_run`, `agent_route execute=true` e jornadas — política de produção, gate e ledger sempre antes de qualquer processo; `agent_route` nunca executa com `needs_review ≥ 0,70`. A aprovação **não volta pela MCP**: `journey_run` grava `~/.config/macrix/approvals/<journey_id>.approval` (0600) com o manifesto completo (tarefa, workspace, yolo, timeout, faixa) e o token; quem tem acesso ao Mac lê e chama `journey_approve` com os mesmos parâmetros, ou o token morre. Dedupe de `op_id` é para sempre (persistido), estado gravado antes de admitir ou consumir (falha de disco recusa a execução). Timeout mata a árvore inteira (`ps` recursivo, TERM→KILL) e a recuperação após reinício mata sobreviventes pelo pid gravado; órfão vivo bloqueia novas admissões. HTTP: bind explícito em 127.0.0.1, `Content-Length` negativo/acima de 1 MiB → 400, `Origin` fora da lista → 403, e pelo túnel (cabeçalhos `cf-*`) só a vitrine responde: `/health`, `/`, `/catalog` e tools `catalog_*`, `jev_*`, `agents_list`, `agents_gate`, `usage_status`, `time_now`, `meta_*`; `/usage` e as tools operacionais só no endpoint local.

**Isolamento (v0.33):** as faixas rodam sob `sandbox-exec` (seatbelt) com `deny file-read*/write*` em `MACRIX_HOME` (aprovações, chaves, ledger) e em `~/.config/frota` — um agente não lê o token e não se autoaprova; `MACRIX_NO_SANDBOX=1` desliga. A vitrine pública é um **listener separado** (`--public-port`, padrão 35731), alvo do túnel, com allowlist exata `catalog_search`, `catalog_stats`, `jev_ping`, `jev_models`, `time_now` — cabeçalho forjado não dá privilégio nenhum. No ledger, a conta é de **tentativas** (tenant obrigatoriamente `*-attempts`), `cost_status` fica sempre `unknown` para CLIs e `attempts_status` diz o que o ledger registrou; resposta incompleta do bridge é erro, não saldo zero; `ledger.json` presente e inválido recusa qualquer execução. `MACRIX_HOME` redireciona todo o estado (instância descartável para provas). `env_inventory` é o censo do que os agentes têm aqui: MCPs, skills, commands, plugins, hooks, agentes do opencode, perfis do codex, skills do muse (só nomes).
